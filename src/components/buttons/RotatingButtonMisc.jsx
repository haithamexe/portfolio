...
//       <Text
//         position={[0, 0, 0.14]}
//         fontSize={frontText.length <= 7 ? 0.15 : 0.1}
//         fontWeight={"bold"}
...
// export default function RotatingButtonMisc() {
//   const { theme } = useThemeContext();
//   const { setButtonFunction } = useButtonContext();

//   const buttons = miscButtons.map((element, i) => ({
//     ...element,
//     position: [
...
export default function RotatingButtonMisc() {
  ...
  useEffect(() => {
    ...
    const loadTextures = async () => {
      const promises = miscButtons.map((button) =>
        useTexture.preload(button.imageUrl)
      );
      const loadedTextures = await Promise.all(promises);
      setTextures(loadedTextures);
      setLoading(false); // All textures are loaded
    };
